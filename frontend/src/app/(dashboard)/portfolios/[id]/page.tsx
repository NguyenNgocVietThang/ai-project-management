...
import { usePortfolioHealth } from '@/features/dashboard/hooks/useDashboard'
...
export default function PortfolioDetailPage() {
  ...
  const health = usePortfolioHealth(id)
  ...
  return <div className="space-y-6">
    ...
    {health.data && <section className="rounded-xl border bg-card p-5"><h2 className="mb-4 text-xl font-semibold">Portfolio health</h2><div className="grid gap-3 sm:grid-cols-3"><Metric icon={<FolderKanban className="h-5 w-5" />} label="Active" value={health.data.active_projects.toString()} /><Metric icon={<FolderKanban className="h-5 w-5" />} label="Completed" value={health.data.completed_projects.toString()} /><Metric icon={<CalendarDays className="h-5 w-5" />} label="Overall progress" value={`${Math.round(health.data.overall_progress)}%`} /></div>{health.data.projects.length > 0 && <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[560px] text-sm"><thead><tr className="border-b text-left text-xs text-muted-foreground"><th className="p-2">Project</th><th className="p-2">Progress</th><th className="p-2">Overdue</th><th className="p-2">Budget used</th></tr></thead><tbody>{health.data.projects.map(item => <tr key={item.project_id} className="border-b last:border-0"><td className="p-2 font-medium">{item.project_name}</td><td className="p-2">{Math.round(item.progress_percent)}%</td><td className="p-2">{item.overdue_tasks > 0 ? <span className="text-destructive">{item.overdue_tasks}</span> : '0'}</td><td className="p-2">{item.budget_utilization_pct === null ? '—' : `${Math.round(item.budget_utilization_pct)}%`}</td></tr>)}</tbody></table></div>}</section>}
