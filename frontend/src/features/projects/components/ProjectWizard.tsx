...
import type { ProjectCreate, RoleSummary, UserSummary } from '@/types/project.types'

export interface InitialProjectMember { user: UserSummary; role: RoleSummary }
...
export function ProjectWizard({ portfolios, onSubmit, onCancel, isLoading, error }: { portfolios: Portfolio[]; onSubmit: (body: ProjectCreate, members: InitialProjectMember[]) => Promise<void>; onCancel: () => void; isLoading: boolean; error?: string | null }) {
  ...
  const addMember = (candidate: UserSummary) => {
  ...
  return <form noValidate onSubmit={submit} className="space-y-6">
    ...
    {step === 2 && <div className="space-y-4">
      ...
      {search && <div className="max-h-44 overflow-y-auto rounded-lg border">{users.data?.map((candidate) => <button key={candidate.id} type="button" onClick={() => addMember(candidate)} className="flex w-full items-center justify-between border-b px-4 py-3 text-left last:border-0 hover:bg-accent"><span><span className="block text-sm font-medium">{candidate.full_name}</span><span className="text-xs text-muted-foreground">{candidate.email}</span></span><UserPlus className="h-4 w-4" /></button>)}{users.data?.length === 0 && <p className="p-4 text-sm text-muted-foreground">No users found.</p>}</div>}
      <div className="space-y-2">{members.map((member) => <div key={member.user.id} className="flex flex-col gap-3 rounded-lg border p-3 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{member.user.full_name}</p><p className="truncate text-xs text-muted-foreground">{member.user.email}</p></div><select value={member.role.id} onChange={(event) => { const role = roles.data?.find((item) => item.id === Number(event.target.value)); if (role) setMembers((current) => current.map((item) => item.user.id === member.user.id ? { ...item, role } : item)) }} className="h-10 rounded-md border bg-background px-3 text-sm">{roles.data?.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</select><button type="button" onClick={() => setMembers((current) => current.filter((item) => item.user.id !== member.user.id))} className="rounded-md p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" aria-label={`Remove ${member.user.full_name}`}><X className="h-4 w-4" /></button></div>)}</div>
