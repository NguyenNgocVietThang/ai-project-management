...
import type { ProjectMemberCreate, UserSearchResult } from '@/types/project.types'
...
export function InviteMemberDialog({ open, onClose, onInvite, isLoading, error }: { open: boolean; onClose: () => void; onInvite: (body: ProjectMemberCreate) => Promise<void>; isLoading: boolean; error?: string | null }) {
  ...
  return <Modal open={open} onClose={close} title="Invite project member" description="Choose an active user and assign their project role.">
    <div className="space-y-5">
      ...
      {query && !selected && <div className="max-h-56 overflow-y-auto rounded-lg border">{users.data?.map((candidate) => <button key={candidate.id} type="button" onClick={() => setSelected(candidate)} className="flex w-full items-center gap-3 border-b p-3 text-left last:border-0 hover:bg-accent"><Avatar name={candidate.full_name} src={candidate.avatar_url} /><span><span className="block text-sm font-medium">{candidate.full_name}</span><span className="text-xs text-muted-foreground">{candidate.email_hint}</span></span></button>)}{users.data?.length === 0 && <p className="p-4 text-sm text-muted-foreground">No active users found.</p>}</div>}
      {selected && <div className="flex items-center gap-3 rounded-lg border bg-muted/30 p-4"><Avatar name={selected.full_name} src={selected.avatar_url} /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{selected.full_name}</p><p className="truncate text-xs text-muted-foreground">{selected.email_hint}</p></div><Button type="button" variant="ghost" className="w-auto" onClick={() => setSelected(null)}>Change</Button></div>}
