...
export function ProjectFields({ register, setValue, errors, portfolios, includeStatus = false }: { register: UseFormRegister<ProjectFormValues>; setValue: UseFormSetValue<ProjectFormValues>; errors: FieldErrors<ProjectFormValues>; portfolios: Portfolio[]; includeStatus?: boolean }) {
  ...
  return <>
    <div><Label htmlFor="project-name">Name</Label><Input id="project-name" hasError={Boolean(errors.name)} {...register('name')} />{errors.name && <p className="mt-1 text-sm text-destructive">{errors.name.message}</p>}</div>
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      <div><Label htmlFor="project-start">Start date</Label><Input id="project-start" type="date" hasError={Boolean(errors.start_date)} {...startDate} onInput={(event) => setValue('start_date', event.currentTarget.value, { shouldDirty: true })} />{errors.start_date && <p className="mt-1 text-sm text-destructive">{errors.start_date.message}</p>}</div>
      <div><Label htmlFor="project-end">End date</Label><Input id="project-end" type="date" hasError={Boolean(errors.end_date)} {...endDate} onInput={(event) => setValue('end_date', event.currentTarget.value, { shouldDirty: true })} />{errors.end_date && <p className="mt-1 text-sm text-destructive">{errors.end_date.message}</p>}</div>
    </div>
    <div className="grid gap-4 sm:grid-cols-2">
      <div><Label htmlFor="project-budget">Budget</Label><Input id="project-budget" inputMode="decimal" hasError={Boolean(errors.budget)} {...register('budget')} />{errors.budget && <p className="mt-1 text-sm text-destructive">{errors.budget.message}</p>}</div>
