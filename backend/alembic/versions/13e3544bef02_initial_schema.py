...
def upgrade() -> None:
    # ### các lệnh được Alembic tự động sinh ra - vui lòng điều chỉnh! ###
...
def downgrade() -> None:
    ...
    # ### kết thúc các lệnh Alembic ###
    # Autogenerate không phát sinh DROP TYPE cho các enum native của PostgreSQL (một
    # hạn chế đã biết của Alembic) - xóa chúng một cách tường minh để downgrade thực sự
    # là nghịch đảo của upgrade và migration có thể được áp dụng lại một cách sạch sẽ.
    enum_types = (
