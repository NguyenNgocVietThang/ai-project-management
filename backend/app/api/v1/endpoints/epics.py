...
async def create_epic(project_id: int, body: EpicCreate, service: WBSServiceDep, current_user: CurrentVerifiedUser):
...
async def update_epic(epic_id: int, body: EpicUpdate, service: WBSServiceDep, current_user: CurrentVerifiedUser):
    return await service.update_epic(epic_id, body, current_user)


@router.delete("/epics/{epic_id}", status_code=204)
async def delete_epic(epic_id: int, service: WBSServiceDep, current_user: CurrentVerifiedUser):
