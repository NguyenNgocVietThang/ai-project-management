...
async def create_sprint(phase_id: int, body: SprintCreate, service: WBSServiceDep, current_user: CurrentUser):
...
async def update_sprint(sprint_id: int, body: SprintUpdate, service: WBSServiceDep, current_user: CurrentUser):
    return await service.update_sprint(sprint_id, body, current_user)


@router.delete("/sprints/{sprint_id}", status_code=204)
async def delete_sprint(sprint_id: int, service: WBSServiceDep, current_user: CurrentUser):
...
async def start_sprint(sprint_id: int, service: WBSServiceDep, current_user: CurrentUser):
...
async def complete_sprint(sprint_id: int, service: WBSServiceDep, current_user: CurrentUser):
