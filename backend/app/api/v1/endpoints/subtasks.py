...
from app.core.dependencies import CurrentVerifiedUser
...
async def update_subtask(subtask_id: int, body: SubtaskUpdate, service: TaskServiceDep, current_user: CurrentVerifiedUser):
    return await service.update_subtask(subtask_id, body, current_user)


@router.delete("/subtasks/{subtask_id}", status_code=204)
async def delete_subtask(subtask_id: int, service: TaskServiceDep, current_user: CurrentVerifiedUser):
